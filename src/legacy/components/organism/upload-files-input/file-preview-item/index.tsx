...
export function FilePreviewItem(props: PropsFileItem) {
  ...
  return (
    <div
      key={index}
      className="flex flex-col rounded-md border-border-default border-2 "
    >
      ...
      <div className="flex flex-row justify-center p-2  max-w-40 max-h-40">
        {IMAGES_MIME.includes(file.type) ||
            IMAGES_MIME.some((imageMime) =>
              imageMime.split("/")[1] === file.type
            )
          ? (
            <img
              src={(file as any)?.url ?? URL.createObjectURL(file)}
              alt={file.name}
