...
export function Show<T>(props: {
  ...
  const content = fn ? (child as any)(condition) : child;

  // If keepState is enabled, render both but hide with CSS
  if (props.keepState) {
    return (
      <Fragment>
        ...
        {!condition && (
          <div style={{ display: "contents" }}>{props.fallback ?? null}</div>
        )}
      </Fragment>
    );
  }

  // Default behavior: mount/unmount
  if (condition) {
